class ChoosePokemonSprite < IconSprite
  ...
  def initialize(size, viewport, style)
    super(viewport)
    @pokemon = nil
    @size = size
    @style = style
    ...
    y_offset = @style == "hp" ? 0 : 64
    if @size == 1
      self.src_rect = Rect.new(0, y_offset, 264, 64)
    elsif @size == 0
      self.src_rect = Rect.new(264, y_offset, 128, 64)
    end
    @icon = PokemonIconSprite.new(nil, viewport)
    @item = IconSprite.new(0, 0, viewport)
    @item.setBitmap("Graphics/UI/Party/icon_item")
    @item.visible = true
  ...
  def refresh
    ...
    if @style == "hp" && !@pokemon.egg?
      ...
      for i in 0...4
        ...
        @overlay.bitmap.fill_rect(112, 40+i*2, [(@hp*1.0*bar_len/@totalhp/2).ceil*2,bar_len-i*2].min, 2, hpcolors[hpzone*2 + color_index])
      end
      textpos.push([sprintf("%d/%d", @pokemon.hp, @pokemon.totalhp), 158, 34, 2, Color.new(252, 252, 252), Color.new(0, 0, 0), 1])
    elsif @style == "use"
      ...
      if @pokemon.item
        textpos.push([GameData::Item.get(@pokemon.item).name, 74, 36, 0, Color.new(252, 252, 252), Color.new(0, 0, 0)])
      end
    end
    ...
    pbDrawTextPositions(@overlay.bitmap,smalltextpos)
  end
...
class ChooseMemberSprite < IconSprite
  ...
  def initialize(size, viewport)
    ...
    if @size == 1
      self.src_rect = Rect.new(0, 0, 264, 64)
    elsif @size == 0
      self.src_rect = Rect.new(264, 0, 128, 64)
    end
  ...
  def member=(value)
    ...
    if @size == 1
      ...
      self.src_rect = Rect.new(0, [64 * value,0].max, 264, 64)
    elsif @size == 0
      self.src_rect = Rect.new(264, [64 * value,0].max, 128, 64)
    end
    if @member == -1
...
class ChoosePokemonScreen
  ...
  def initialize(mode, z = nil, style = "hp")
    @mode = mode
    @style = style
    @member = 0
    @index = 0
  ...
  def pbStartScreen
    ...
    for x in 0...3
      ...
      for y in 0...3
        pokemon = ChoosePokemonSprite.new(x % 2, @viewport, style = @style)
        pokemon.x = x_pos[x]
...
def pbChoosePokemonScreen(mode = 0, z = nil, style = "hp")
  screen = ChoosePokemonScreen.new(mode, z, style)
